...
def test_skip_tf_plan(mock_tf_test):
    '''Ensures that the `terraform plan` command is not executed and tests that depend on and/or use the tf_plan fixture are skipped'''
    plugin = JSONReport()
...
def test_skip_tf_apply(mock_tf_test):
    '''Ensures that the `terraform apply` command is not executed and tests that depend on and/or use the tf_apply fixture are skipped'''
    plugin = JSONReport()
    os.chdir(f'{os.path.dirname(__file__)}/fixtures')
...
def test_skip_tf_destroy(mock_tf_test):
    '''Ensures that the `terraform destroy` command is not executed on teardown'''
...
def test_skip_tf_destroy_backend_preserved():
    '''Ensures that the Terraform configuration's tfstate file still exists after the Pytest execution is finished'''
    tf_dir = f'{os.path.dirname(__file__)}/fixtures'
    expected_tf_state_path = f'{os.path.dirname(__file__)}/fixtures/module/terraform.tfstate'
    os.chdir(tf_dir)
    try:
        pytest.main(['--skip-tf-destroy', '--json-report-file=none', 'test_tf.py'])
        log.info(f'Assert that the tfstate file exists: {expected_tf_state_path}')
        assert os.path.exists(expected_tf_state_path) == True
