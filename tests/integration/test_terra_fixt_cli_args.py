...
def test_skip_tf_plan(mock_tf_test):
    plugin = JSONReport()
    os.chdir(f'{os.path.dirname(__file__)}/../unit')
    pytest.main(['--skip-tf-plan', '--json-report-file=none', 'test_terra_fixt.py'], plugins=[plugin])
...
def test_skip_tf_apply(mock_tf_test):
    plugin = JSONReport()
    os.chdir(f'{os.path.dirname(__file__)}/../unit')
    pytest.main(['--skip-tf-apply', '--json-report-file=none', 'test_terra_fixt.py'], plugins=[plugin])
    summary = plugin.report['summary']
    log.debug(f'Test Summary:\n{summary}')
...
def test_skip_tf_destroy(mock_tf_test):
    ...
    pytest.main(['--skip-tf-destroy', '--json-report-file=none', 'test_terra_fixt.py'], plugins=[plugin])
    summary = plugin.report['summary']
    log.debug(f'Test Summary:\n{summary}')
